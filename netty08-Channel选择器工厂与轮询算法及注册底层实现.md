...
- 由以上可知，Channel上的那些ChannelHandler的处理全部是在一个线程中处理，这也就是为什么遇到逻辑比较复杂的ChannelHandler一定要单独起一个专门的业务线程池（EventExecutorGroup），否则会造成线程阻塞。业务线程池通常有2种实现方式：
> 在ChannelHandler的回调方法中，使用定义的业务线程池，这样九可以实现异步调用。
```java
...
```
> 借助Netty提供的向ChannelPipeline添加ChnanelHandler时调用的addLast方法来传递EventExecutor：
```java
